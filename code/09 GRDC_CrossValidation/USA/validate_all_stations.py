"""
GRDC Cross-Validation: USA stations
Generate per-station time series comparison plots for all stations with overlap.
"""
...
OUTDIR = ROOT / "09 GRDC交叉验证" / "USA" / "plots"
...
for _, row in match_df.iterrows():
    ...
    # zoom to +/- 12 months around filled period
    ...
    fname = f"val_{grdc_id}_{gsim_id}_{grdc_name.replace(' ', '_').replace('/', '_').replace(',', '')}"
...
print(f"\n=== Summary: {len(sdf)} stations (sorted by NSE on filled points) ===")

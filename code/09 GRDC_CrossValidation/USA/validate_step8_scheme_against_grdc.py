...
GRDC_SHP = Path(os.environ.get("GSIM_PLUS_USA_GRDC_SHP", EXTERNAL / "grdc_stations.shp"))
GSIM_SHP = Path(os.environ.get("GSIM_PLUS_USA_GSIM_SHP", EXTERNAL / "gsim_stations.shp"))


def parse_args():
    parser = argparse.ArgumentParser(description="Validate a Step 8 GSIM fill directory against USA GRDC observations.")
    parser.add_argument("--gsim-fill-dir", required=True, help="Directory containing per-station GSIM fill CSV files.")
    parser.add_argument("--outroot", required=True, help="Output directory for plots and validation summary.")
    parser.add_argument("--label", default="GSIM-PLUS", help="Legend label for the validated product.")
...
def build_match_df():
    gdf1 = gpd.read_file(GRDC_SHP)
    gdf2 = gpd.read_file(GSIM_SHP)
    ...
    return pd.DataFrame(match_pairs)


def main():
    args = parse_args()
    gsim_fill_dir = Path(args.gsim_fill_dir)
    ...
    for _, row in match_df.iterrows():
        ...
        gsim_path = gsim_fill_dir / f"{gsim_id}.csv"
        ...
        ax1.plot(zoom["date"], zoom["MEAN"], color="#3C5488", lw=1.0, alpha=0.9, label="GRDC observed", zorder=2)
        ax1.plot(zoom["date"], zoom["final_streamflow"], color="#E64B35", lw=1.0, alpha=0.9, label=args.label, zorder=3)
        ...
        safe_name = grdc_name.replace(" ", "_").replace("/", "_").replace(",", "").replace("(", "").replace(")", "")
    ...
    sdf.to_csv(outroot / "validation_summary.csv", index=False)
    print(f"\nSaved {len(sdf)} stations to {outroot / 'validation_summary.csv'}")

...
Figure 4 — Continuous Gap Degradation (Line charts)
  ...
  MAML & DTRR plotted last (on top) with thicker lines for emphasis.
"""
...
# ── config ───────────────────────────────────────────────────────────────
# Draw other methods first, MAML & DTRR last (on top)
OTHER_METHODS = ["RandomForest", "Linear",
                 "LSTM", "KNN", "SeasonalMean", "IDW"]
HIGHLIGHT_METHODS = ["MAML", "DTRR"]
...
for metric, suffix in [("NSE", "a"), ("KGE", "b")]:
    ...
    # draw MAML & DTRR last — thicker, fully opaque, higher zorder
    ...
    # legend: DTRR first, then MAML, then others — placed outside top
    handles, labels = ax.get_legend_handles_labels()
    dt_idx = labels.index("DTRR")

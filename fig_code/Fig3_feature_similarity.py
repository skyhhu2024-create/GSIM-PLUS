...
# ══════════════════════════════════════════════════════════════════════════
fig_a, ax = plt.subplots(figsize=(85 / 25.4, 80 / 25.4), dpi=300)

colors = [GROUP_COLORS.get(g, "#CCCCCC") for g in wdf["group"]]  # NaN 用灰色代替
...
legend_elements = [Patch(facecolor=GROUP_COLORS[g], label=f"{g}")
                   for g in ["Climate", "Spatial", "Topography", "Soil"]]

...
for csv_file in csv_files:
    ...
    fill_mask = (
        df['fill_method'].ne('OBSERVED')
        & np.isfinite(df['gsim_filled'])
        & np.isfinite(df['grdc_observed'])
    )
    ...
    if fill_mask.sum() >= 1:
        ...
        r = np.corrcoef(gsim_v, grdc_v)[0, 1] if len(gsim_v) > 1 else np.nan
        r2 = r ** 2
        denominator = np.sum((grdc_v - np.mean(grdc_v))**2)
        nse = 1 - np.sum((gsim_v - grdc_v)**2) / denominator if denominator > 0 else np.nan
        ...
        results.append({
            ...
            'Region': REGIONS.get(station_label[:2], 'Unknown'),
            'Evaluated_infilled_months': n_filled,
            'Mean_flow': mean_flow,
            'Fill_NSE': nse,
            'Fill_R': r,
            'Fill_R2': r2,
...
results_df.to_csv(DATADIR / "16stations_metrics.csv", index=False, float_format='%.4f')

...
# ── colors (NPG style) ──────────────────────────────────────────────────
C_ANC = "#0072B2"   # Anchor - blue
C_TAR = "#E69F00"   # Target - orange
C_INS = "#999999"   # Insufficient - neutral grey
...
# ── (a) All GSIM stations (1995-2015) ──────────────────────────────────
all_geo = all_st.merge(coord, on="station_id", how="left").dropna(subset=["latitude"])
fig_a, ax_a = plt.subplots(figsize=(180 / 25.4, 90 / 25.4), dpi=600,
                            subplot_kw={"projection": proj})
style_map(ax_a)
ax_a.scatter(
    all_geo["longitude"], all_geo["latitude"],
    s=0.4, c="#3C5488", alpha=0.35, linewidths=0,
    transform=pc, zorder=2, rasterized=True,
)
ax_a.text(
    0.02, 0.06, f"All GSIM stations (n = {len(all_st):,})",
    transform=ax_a.transAxes, fontsize=8, color="#333333",
    bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#cccccc", lw=0.3, alpha=0.9),
)
fig_a.savefig(str(OUTDIR / "Fig1a_all_stations.png"), dpi=600, bbox_inches="tight", facecolor="white")
fig_a.savefig(str(OUTDIR / "Fig1a_all_stations.pdf"), bbox_inches="tight", facecolor="white")
plt.close(fig_a)
print("Saved Fig1a")

# ── (b) Anchor / Target / Insufficient map ──────────────────────────────
fig_b, ax_b = plt.subplots(figsize=(180 / 25.4, 90 / 25.4), dpi=600,
                            subplot_kw={"projection": proj})
style_map(ax_b)
...
ax_b.legend(loc="lower left", markerscale=10, handletextpad=0.3,
            borderpad=0.4, labelspacing=0.3, fontsize=8)

fig_b.savefig(str(OUTDIR / "Fig1b_anchor_target.png"), dpi=600, bbox_inches="tight", facecolor="white")
fig_b.savefig(str(OUTDIR / "Fig1b_anchor_target.pdf"), bbox_inches="tight", facecolor="white")
plt.close(fig_b)
print("Saved Fig1b")

# ── (c) Histogram ───────────────────────────────────────────────────────
fig_c, ax_c = plt.subplots(figsize=(130 / 25.4, 55 / 25.4), dpi=600)
...
ax_c.text(0.105, ax_c.get_ylim()[1] * 0.93, "10%", fontsize=8, color=C_ANC, va="top")
ax_c.text(0.705, ax_c.get_ylim()[1] * 0.93, "70%", fontsize=8, color=C_TAR, va="top")

ax_c.set_xlabel("Missing fraction (1995–2015)")
ax_c.set_ylabel("Number of stations")
ax_c.legend(loc="upper center", fontsize=8)
ax_c.set_xlim(-0.02, 1.02)
ax_c.spines["top"].set_visible(False)
ax_c.spines["right"].set_visible(False)

fig_c.savefig(str(OUTDIR / "Fig1c_histogram.png"), dpi=600, bbox_inches="tight", facecolor="white")
fig_c.savefig(str(OUTDIR / "Fig1c_histogram.pdf"), bbox_inches="tight", facecolor="white")
plt.close(fig_c)
print("Saved Fig1c")

# ── (d) CDF ─────────────────────────────────────────────────────────────
fig_d, ax_d = plt.subplots(figsize=(130 / 25.4, 55 / 25.4), dpi=600)
...
fig_d.savefig(str(OUTDIR / "Fig1d_cdf.png"), dpi=600, bbox_inches="tight", facecolor="white")
